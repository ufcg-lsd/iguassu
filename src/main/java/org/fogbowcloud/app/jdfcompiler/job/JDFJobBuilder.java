...
public class JDFJobBuilder {
     ...
     */
    public void createJobFromJDFFile(JDFJob job, String jdfFilePath, JobSpecification jobSpec,
        String userName,
        ...
        if (file.exists()) {
            ...
            throw new IllegalArgumentException(
                "File: " + file.getAbsolutePath() + " does not exists.");
        }
    }
        ...
        for (String req : jobRequirements.split(REQUIREMENTS_SEPARATOR)) {
            if (req != null && !req.isEmpty()) {
                if (req.startsWith(DockerConstants.PREFIX_DOCKER_REQUIREMENTS)) {
                    String requirements = spec
                        .getRequirementValue(DockerConstants.METADATA_DOCKER_REQUIREMENTS);
                    if (requirements == null) {
                        spec.addRequirement(DockerConstants.METADATA_DOCKER_REQUIREMENTS, req);
                    } else {
                        spec.addRequirement(DockerConstants.METADATA_DOCKER_REQUIREMENTS,
    ...
    private void addIOCommands(String jobId, Task task, IOCommand command, String userName,
        ...
        switch (IOType) {
            case "PUT":
            case "STORE":
                Command uploadFileCommand = uploadFileCommands(sourceFile, destination, userName,
                ...
                break;
            case "GET":
                Command downloadFileCommand = downloadFileCommands(sourceFile, destination,
                    userName, externalOAuthToken, tokenVersion, rawCommand);
    ...
    private void addRemoteCommand(String jobId, Task task, RemoteCommand remCommand) {
        ...
        LOGGER.debug("JobId: " + jobId + " task: " + task.getId() + " remote command: " + remCommand
            .getContent());
    ...
    private String getRefreshTokenCommand(String userId, Long tokenVersion) {
        ...
        final String refreshTokenUrl = String
            .format("%s/auth/oauth2/refresh/${userId}/${tokenVersion}", iguassuUrl, userId, tokenVersion);
