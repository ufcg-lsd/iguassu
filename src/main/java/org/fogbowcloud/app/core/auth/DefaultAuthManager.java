...
public class DefaultAuthManager implements AuthManager {
	...
	@Override
	public User store(String username, String secretToken) {
		try {
			DefaultUser user = new DefaultUser(username, secretToken);
	...
	public void update(User currentUser) {
		final User outdatedUser =
			DefaultUser.fromJSON(new JSONObject(this.userList.get(currentUser.getIdentifier())));

		if (!outdatedUser.equals(currentUser)) {
			logger.debug("Updating user: " + this.userList.remove(currentUser.getIdentifier()));
			this.userList.put(currentUser.getIdentifier(), DefaultUser.toJSON(currentUser).toString());
			this.usersDB.commit();
