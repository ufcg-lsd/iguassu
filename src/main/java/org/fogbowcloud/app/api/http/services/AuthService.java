...
public class AuthService {
	...
	public AuthDTO authenticate(String authorizationCode, String applicationIdentifiers)
		...
		if (isAReliableApp(applicationIds)) {
			...
			final String baseUrl =
				this.properties.getProperty(ConfProperties.OAUTH_STORAGE_SERVICE_TOKEN_URL);
			...
			mountsHeaders(
				headers, encodeHeaders(applicationIds.getClientAppId(), applicationIds.getSecret()));
	...
	private boolean isAReliableApp(OAuthIdentifiers applicationIds) {
		...
		final String knownSecret =
			this.properties.getProperty(ConfProperties.OAUTH_STORAGE_SERVICE_CLIENT_SECRET);
	...
	private void mountsAuthorizationHeader(List<Header> headers) {
		String clientId = this.properties.getProperty(ConfProperties.OAUTH_STORAGE_SERVICE_CLIENT_ID);
		String clientSecret =
			this.properties.getProperty(ConfProperties.OAUTH_STORAGE_SERVICE_CLIENT_SECRET);
			...
			if (oAuthTokenRawResponse != null) {
				...
				if (Objects.nonNull(user)) {
					...
					if (user.isActive()) {
						...
						logger.debug(
							"User [" + user.getIdentifier() + "] is active and has a valid Iguassu Token.");
					} else {
						iguassuToken = this.generateIguassuToken(oAuthToken.getUserId());
						logger.debug(
							"Generating a new Iguassu Token for the user [" + user.getIdentifier() + "].");
